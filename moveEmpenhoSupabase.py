"""
Script: moveEmpenhoSupabase.py
Objetivo: Mover dados de empenho do MongoDB (estoque_centralizado) para PostgreSQL (Supabase)
Ambientes:
  - Origem: MongoDB (estoque_centralizado.empenho)
  - Destino: PostgreSQL/Supabase (estoquecentralizado.empenho)
Versao: 1.0
"""
...
MONGO_ORIGEM = os.environ.get(
    "MONGO_ORIGEM_URL",
    "mongodb+srv://mateus_lemos:pXqgrqsWz0nqOqQtV588@estoque-centralizado.gwuokd.mongodb.net/?"
)
BASE_DESTINO = "estoque_centralizado"
COLLECTION_ORIGEM = "empenho"

# ========== CONFIGURACOES SUPABASE/POSTGRES ==========
# Defina a variavel de ambiente SUPABASE_DATABASE_URL ou configure abaixo.
SUPABASE_DATABASE_URL = os.environ.get(
    "SUPABASE_DATABASE_URL",
    "postgresql://postgres.ulpppmzsxjbsqiyfskwi:estoquecentralizado@aws-1-sa-east-1.pooler.supabase.com:6543/postgres"
)
POSTGRES_SCHEMA = "estoquecentralizado"
POSTGRES_TABLE = "empenho"
...
def conectar_mongodb():
    ...
    try:
        ...
        client = MongoClient(MONGO_ORIGEM, serverSelectionTimeoutMS=5000)
...
def inserir_lote_postgres(conn, documentos_lote):
    ...
    upsert_query = sql.SQL(
        """
        INSERT INTO {}.{} (
            id, lojaid, produtoid, saldo, dt_criacao, dt_atualizacao
        )
        VALUES (
            %(id)s, %(lojaid)s, %(produtoid)s, %(saldo)s, NOW(), NULL
        )
...
def main():
    logger.info("="*80)
    logger.info("MIGRACAO DE DADOS - Empenho de Estoque (MongoDB -> Supabase/PostgreSQL)")
