"""
Script: moveLoteSupabase.py
Objetivo: Mover dados de lotes do MongoDB (estoque_centralizado) para PostgreSQL (Supabase)
Ambientes:
  - Origem: MongoDB HML OnPremise (estoque_centralizado.lote)
  - Destino: PostgreSQL/Supabase (estoquecentralizado.lote)
Versao: 1.0
"""
...
def converter_para_postgres(doc_mongo):
    ...
    try:
        return {
            ...
            'numero': str(doc_mongo.get('numero', '')),
            'saldo': _to_int(doc_mongo.get('saldo', 0)),
            'dtfabricacao': _to_date(doc_mongo.get('dtfabricacao')),
            'dtvencimento': _to_date(doc_mongo.get('dtvencimento')),
            'data_referencia': datetime.date.today(),
            'nrregms': str(doc_mongo.get('nrregms')) if doc_mongo.get('nrregms') else None,
        }
    except Exception as e:
...
def inserir_lote_postgres(conn, documentos_lote):
    ...
    upsert_query = sql.SQL(
        """
        INSERT INTO {}.{} (
            id, lojaid, produtoid, numero, saldo,
            dtfabricacao, dtvencimento, data_referencia, nrregms,
            dt_criacao, dt_atualizacao
        )
        VALUES (
            %(id)s, %(lojaid)s, %(produtoid)s, %(numero)s, %(saldo)s,
            %(dtfabricacao)s, %(dtvencimento)s, %(data_referencia)s, %(nrregms)s,
            NOW(), NULL
        )
        ON CONFLICT (id) DO UPDATE SET
            ...
            dtfabricacao = EXCLUDED.dtfabricacao,
            dtvencimento = EXCLUDED.dtvencimento,
            data_referencia = EXCLUDED.data_referencia,
            nrregms = EXCLUDED.nrregms,
            dt_atualizacao = NOW();
        """
    ).format(sql.Identifier(POSTGRES_SCHEMA), sql.Identifier(POSTGRES_TABLE))
...
def main():
    logger.info("="*80)
    logger.info("MIGRACAO DE DADOS - Lotes de Estoque (MongoDB -> Supabase/PostgreSQL)")
